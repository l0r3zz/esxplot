...
#
# esxplot - explore and plot data generated by the esxtop utility when run in 'batch' mode.
# This program uses the cross platform wxPython Window System so it should run on any Windows,
# Linux, or Mac OSX system that has Python >= 2.6 and with wxPython >= 2.8 installed

version = "esxplot v1.1.5-04152010"
__svnid__ = '$Id: $'
copyright = """
...
class MyFrame(esxp_gui.EsxPlotFrame):
    '''This is the main "work horse" class. Most of the methods called here do the brunt of the
       displaying and receive all of the input.
    '''
    def __init__(self, parent, id, title, csvl):
        ...
        self.datavector = csvl  #current dataset object reference (from DataSource)
                                # this might become an array in future versions that deal with multiple datasets
        ...
        self.exp_column_dir = ""  # Holds the last directory specified by user on CSV export
        self.itemlist =[] # array to hold multiple selections
        
        self.default_color = ( 'midnightblue','red','thistle','brown','purple','goldenrod','orangered','springgreen',
                      'steelblue','peru','orange','deeppink','chartreuse','cyan','fuchsia','maroon')
        self.color = list(self.default_color[:])        

        # Bring up the GUI
        # the elements referenced here are defined in esxp_gui_custom.py where the parent class is defined
        ...
        # set up the Grid and tell MyPlotCanvas whether we're running Windows or not
        ...
        self.tree.Bind(wx.EVT_TREE_ITEM_RIGHT_CLICK, self.OnRightClick, id=wx.ID_ANY)            
        #Bind the OnSearchQuery method to the textctrl widget, bind it to the EVT_BUTTON event
        self.queryButton.Bind(wx.EVT_BUTTON,self.OnSearchQuery,id=wx.ID_ANY)
        self.Bind(wx.EVT_CLOSE, self.OnCloseHelp)    #Make sure we clean up any stray windows on the way out
    ...
    def MyAlert(self, message):
        ...
        """
        dlg = wx.MessageDialog(self, message,"Warning!", wx.OK | wx.ICON_INFORMATION)
        #log("Warning!: "+message)
        dlg.ShowModal()
        dlg.Destroy()
        return
...
####################
    ...
    def OnAbout(self, event):
        ...
        """
        about_text = version+"\n"+copyright
        dlg = wx.MessageDialog(self, about_text,"About Me", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()

    def OnPrefs(self,event):
        '''
        Called when the User selects the Preference menu item. Put all preferences, switches, dials and
        knobs, here. Uses the Dialog designed with wxGlade imported from esxp_gui.py
        '''
        def _load_colors():
            '''
        ...
        dlg = esxp_gui.EsxPlotPreferencesDialog(None, -1) # call the Dialog constructor created by wxGlade
        ...
        if dlg.ShowModal()== wx.ID_OK : # If the user pressed OK, there may be preference changes to process
    ...
    def OnHelp(self,event):
        ...
        html = wx.html.HtmlWindow(dlg,pos=(10,10),size=(780,430), style=wx.html.HW_SCROLLBAR_AUTO )
    ...
    def OnImportData(self,event): ### candidate for refactoring for 1.1
        ...
        dlg = wx.FileDialog(self, "esxtop batch output file", self.dirname,"", "*.*", wx.OPEN)
        ...
        if self.filename != '' :
            ...
            try:
                v = DataSource( fpath)        # now that we have a valid filename, let's get this Party started
            except (ValueError,csv.Error)as err:
                self.MyAlert(fpath+" doesn't seem to be an estop data set, "+str(err))
                return
            
            self.datavector = v         # set datavector so other methods can find the info
    ...
    def OnImportQueries(self,event):
        ...
        dirname = ''
        filename = ''
        dlg = wx.FileDialog(self, "Saved Queries file", dirname,"", "*.*", wx.OPEN)
        ...
        if filename != '' :
            ...
            if not re.match("#%%esxplot:queries%%",rawqueries[0]) :
                self.MyAlert(fpath+" doesn't seem to be an saved queries file")
                return
    ...
    def OnExportColumns(self,event):
        ...
        dlg = wx.FileDialog ( None, defaultDir=self.exp_column_dir, style = wx.FD_SAVE|wx.FD_OVERWRITE_PROMPT )
        if dlg.ShowModal() == wx.ID_OK:
            filename=dlg.GetFilename()
            dirname=dlg.GetDirectory()
            ...
            try:
                ...
                self.MyAlert("File "+fpath+" could not be opened for writing, check permissions")
                return
            #open the csv file to write the selections to, make the output reloadable to esxplot
            v = csv.writer(fildes,quoting=csv.QUOTE_ALL,dialect="excel",lineterminator=',\r\n') 
            ...
            for i in xrange(self.datavector.samplemag):
                rowvalues = [str(self.datavector.columns[int(x)][i]) for x in columnlist]
    ...
    def OnSelection(self, event):
        '''
        Method called when selected item is changed ( click on a metric and it will be plotted.
        This is the main routine that does the plotting 
        '''


        selectionlist = [ self.tree.GetPyData(x)  for x in self.tree.GetSelections() if self.tree.GetPyData(x) != None ] #filter out trunk selections
        ...
        if len(selectionlist) > len(self.color) :    #too many selections were made
            self.MyAlert("You can only plot a maximum of "+str(len(self.color))+" metrics in a single graph")
            return
        
        if len(selectionlist) == 1 :
            if (len(self.itemlist) == 1) and (isWindowsG) : #stupid windows implementation sends bogus event on multiple selection
                return
            self.itemlist= selectionlist[:]
            #log("itemlist @ == 1"+str(self.itemlist))
        else :
            #log("Set calculation= "+str(list(set(selectionlist) - set(self.itemlist))))
        ...
        for item in self.itemlist :
            ...
            if columnindex == None : # User selected a non-leaf node, (no data present)
                continue
           
            columnindex = int(columnindex) # Make sure, it's an integer and not a string object
            #reference the column data list, the number of samples and the time quanta from the DataSource object
            ...
            # use a list comprehension with xrange to create the co-ordinate tuples
            data = [(self.datavector.time_axis[x-1], float(columndata[x])) for x in xrange (1, numberofsamples)]
            
            float_data = [float(columndata[x]) for x in xrange(1, numberofsamples)]
            
            # draw points as a line
            line.append(wxPlot.PolyLine(data,legend=legend, colour=self.color[colorindex%len(self.color)], width=self.line_width))
            colorindex +=1
            
            mmm= minmaxavg_fmt.format(labelinfo[3]+"/"+labelinfo[4],min(float_data),max(float_data), sum(float_data)/len(float_data), scoreatpercentile(float_data, 90) ) 
                     
            self.MyTextUpdate(mmm)
        #Label for the X axis
        xaxislabel = 'Time(sec) : Start Time: '+self.datavector.starttime+'  :  End Time: '+\
        self.datavector.endtime+' : Sample period = '+str(timequanta)+' seconds'
    ...
    def OnSearchQuery(self,event):
        """
        Method called when a user types a search query that will be applied to the loaded tree of esxtop metrics
        """
        if self.isDatasetLoaded :
    ...
    def _applyQuery(self,regExString,color="BLUE"):
        """
        This Method actually does the regex search of the dataset and displays the new query
        """

        try:
            regExObject = re.compile(self._raw(regExString.rstrip('\n')), re.IGNORECASE|re.VERBOSE)
        except:
            self.MyAlert("Unrecognizable Regular Expression!")
            return
        query_display = (regExString.strip()).replace('\n',' ')
        result_set_tree = HvTree('%%SearchResult%%') #create a tree to hold the result set
        dlg = wx.ProgressDialog("Search Progress","",maximum =self.datavector.colmag,
        ...
        for j in xrange(self.datavector.colmag-1): # ugly, fix me
            if j % 100 ==0 :
                    dlg.Update(j)
            if regExObject.search(self.datavector.l[j]) == None : # if no match go to the next one
                continue
            
            ### Apply the WHERE clause here if there is one ###
                 
            m = self.datavector.l[j].split('\\')     # turn label into a list of path elements
        ...
        if result_set_tree.isZero() :
            ...
            self.datavector.AddQueryString(regExString)  # save the query in case user wants to save favorites
        dlg.Destroy()
        return
    
    def _raw(self,text):
        """Returns a raw string representation of text"""
    # this is a translation dictionary and function to fix the input a user types into the 
    # regex query area, this enables a user to "cut and paste" regex right out of their
    # egrep scripts and have it work without fiddling with the expression itself
        escape_dict={'\a':r'\a',
               '\b':r'\b',
               '\c':r'\c',
               '\f':r'\f',
    ...
    def OnRightClick(self,event):
        """
        Handle right-clicks on a selection, throw up a context menu with possible options
        """
    ...
    def OnCloseDataset(self,event):
        """
        Close the current dataset and attempt to release all of the created objects and data structures
        """
        if not self.isDatasetLoaded :
            self.MyAlert("No dataset is currently open!")
            return
        self.datavector = None  #might need to perform actions... see DataSource     
        self.tree.DeleteAllItems() #might need to perform actions ... see MyTreeCtrl
        self.plotter.Clear()
        self.isDatasetLoaded = False
...
class MyHelpDialog(wx.Dialog):
    ...
    def __init__(self,html_page=""):
        wx.Dialog.__init__(self, None, -1, 'Help',size=(800, 500),style=wx.CAPTION)
    ...
    def OnDelete(self, event):
        if self.parent.MyDialog("Delete this branch?") :
            ...
            if len(itemlist) > 1 :         #too many selections were made
                self.parent.MyAlert("You can only right click on a single selection")
                return
...
class MyApp(wx.App):
    ...
    def OnInit(self):
        ...
        '''
        #discover what kind of OS we are running on (note: isWindowsG is a global!)
        global isWindowsG
        if os.name == 'nt' :
            isWindowsG = True 
        else :
        ...
        if isWindowsG:
            logfile_path = os.path.expanduser("~")+"\Local Settings\Application Data\esxplot"
        ...
        if logfile_path :
            ...
            try:
                ...
                print("Can't open logfile "+logfile_path+logfile_name+" defaulting to stdout")
                logfiledes = sys.stdout
        else:
            logfiledes = sys.stdout
        ...
        if len(args) == 0 :       #called with no arguments, just start the GUI
            v = None
        else :                          #mustr have a filename argument, check it and read the data
            if os.path.exists(args[0]) == False :
                ...
                try:
                    ...
                    log("?File does not seem to be an esxtop data set - "+args[0])
                    self._errstop()
...
<b>esxplot[.py]
</b> <b>[-d | -l </b> <i>logfile_path</i>  <b>]</b> <b>[</b> <i>data_file</i> <b>] </b> 
<h2><a name='sect2' href='#toc2'>Description</a></h2>
<b>esxplot</b> is a GUI application
that lets you explore the data collected by  <b>esxtop</b> in batch mode. The program
takes a single command line argument which is the esxtop batch mode output
file.  You can also  simply start  <b>esxplot</b> without any arguments, and enter
...
against the metric descriptions for the columns making up the treecontrol.
 Pleae note that you must click the <i>GO</i> button in order to execute the query,
...
we have  <i>About,</i> <i>Import,</i> <i>Export,</i> <i>Close,</i> <i>Preferences,</i> <i>Help</i> and <i>Exit</i> 
<p> <b>About</b>
<blockquote>Displays general information about <b>esxplot</b> including version and license
...
to a file in a particular image format. Allowable formats are  <i>.bmp</i> <i>.xbm</i> <i>.xpm</i>
<i>.png</i>  and <i>.jpg</i>  
...
<p> <b>Right-Mouse-Down on  Selections</b> <blockquote><i>Right-Mouse-Down</i> on a
...
to load a new dataset, you must use the  <b>Close</b> menu item to close the Current
Dataset. 
...
<p> <blockquote>This menu item currently has one element, <i>Current Dataset</i>. Selecting this
...
a dataset that is smaller in size will not demand additional memory. </blockquote>

<p> <b>Preferences</b>

<p> <blockquote>This displays a panel with set-able parameters that control the way esxplot
...
that you don&rsquo;t like. When you click OK on the preferences panel the preferences
...
to the specified logfile and not to the default location, (~/.esxplot/esxplot.log
on *nix systems, \Documents and Settings\&lt;user&gt;\Local Settings\Application Data\esxplot\esxplot.log
...
in Python 2.6, and needs the  <i>wxPython</i> library <i>2.8.10</i> or greater,along with
 the <i>numpy</i> library <i>1.3.0</i> or greater.  <i>WxPython</i> allows you to write GUI programs
...
you&rsquo;ve done something wrong.  Otherwise, if the application dies mysteriously,
...
<p> <a href='http://communities.vmware.com/docs/DOC-9279'>http://communities.vmware.com/docs/DOC-9279</a>
